...
fb_actions.copy = function(prompt_bufnr)
	...
	for _, selection in ipairs(selections) do
		...
		local name = selection:absolute() ~= target_dir and selection.filename:sub(#selection:parent().filename + 2)
			or nil
		...
		if destination:absolute() == selection:absolute() then
			...
			if destination == absolute_path then
				a.nvim_echo({
					{
						string.format(
							"\nSource and target paths are identical for copying %s! Skipping.",
							absolute_path
						),
					},
				}, false, {})
